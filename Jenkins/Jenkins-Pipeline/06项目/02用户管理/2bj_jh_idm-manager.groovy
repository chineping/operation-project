...
pipeline {
    ...
    parameters {
        ...
        string(name:'WarPack', defaultValue: 'smart-idm-manager/build/libs/smart-idm-manager-2.0.0-SNAPSHOT.jar', description: 'gradle构建出的服务jar包')
        ...
        stage ('停服务备份生产包') {
            steps {
                node ( agentLabel as String ) {  // Evaluate the node label later
                    sh returnStatus: true, script: "stop idm-manager"
                    script {
                        def pid = sh returnStdout: true ,script: "ps -ef|grep idm-manager|grep -v grep|awk '{print \$2}'"
                        pid = pid.replaceAll("(\r\n|\r|\n|\n\r)", " "); 
                        // pid = pid.trim()
