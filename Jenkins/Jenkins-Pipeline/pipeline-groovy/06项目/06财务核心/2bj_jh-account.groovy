...
pipeline {
    ...
    parameters {
        ...
        string(name:'WarPack', defaultValue: 'build/libs/smart-services-finance-account-0.1.1.jar', description: 'gradle构建出的服务jar包')
    ...
    stages {
        stage ('拉取代码库') {
            steps {
                git branch: 'master', credentialsId: '06c8476d-ef81-4c84-b1eb-71d341abf97b', url: 'git@gitlabIP:smart/smart-services/smart-services-finance-account.git'
            }
        }
        ...
        stage ('停服务备份生产包') {
            steps {
                node ( agentLabel as String ) {  // Evaluate the node label later
                    ...
                    script {
                        def pid = sh returnStdout: true ,script: "ps -ef|grep finance-account|grep -v grep|awk '{print \$2}'"
                        pid = pid.replaceAll("(\r\n|\r|\n|\n\r)", " "); 
                        // pid = pid.trim()
