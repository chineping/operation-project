...
pipeline {
    ...
    parameters {
        ...
        string(name:'WarPack', defaultValue: 'build/libs/smart-services-credit-server-2.0.0-SNAPSHOT.jar', description: 'gradle构建出的服务jar包')
    ...
    stages {
        stage ('拉取代码库') {
            steps {
                git branch: 'master', credentialsId: '06c8476d-ef81-4c84-b1eb-71d341abf97b', url: 'git@gitlabIP:smart/smart-services/smart-services-credit-server.git'
            }
        }
        ...
        stage ('停服务备份生产包') {
            steps {
                node ( agentLabel as String ) {  // Evaluate the node label later
                    sh returnStatus: true, script: "stop credit-server"
                    script {
                        def pid = sh returnStdout: true ,script: "ps -ef|grep credit-server|grep -v grep|awk '{print \$2}'"
                        pid = pid.replaceAll("(\r\n|\r|\n|\n\r)", " "); 
                        // pid = pid.trim()
                ...
                node ( agentLabel as String ) {
                    script {
                        withEnv (['JENKINS_NODE_COOKIE=dontkillme']) {
                            sh """
                            nohup java -Xms128m -Xmx512m -XX:PermSize=64M -XX:MaxPermSize=128M -jar ${params.proPath}/${WarPackname} --spring.profiles.active=sit > ${params.logPath}/${WarPackname}.log 2>&1 &
                            """
                        }
                    }  
                } 
