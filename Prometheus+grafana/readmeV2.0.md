...
```
1、开发人员利用springboot自带的提供接口：http://IP:8888/prometheus；
2、运维人员在IP上配置promethus一套的服务端；
3、监控 increase(jvm_memory_pool_bytes_used{pool="PS   Old Gen"}[2m]) / jvm_memory_pool_bytes_max{pool="PS Old Gen"} * 100 > 70
2min老生代内存使用的增长量/最大 *100% >50%参数，60秒内内存占用增加70%触发告警
```

修改promethus.yml配置文件，新增以下字段

```
  - job_name: 'xkbank-metrics'
    metrics_path: /prometheus
    scrape_interval: 5s
    static_configs:
      - targets: ['IP1_q-xk-frtserver01:9888','IP2_q-xk-frtserver02:9888']
```

新增一个jvm的告警规则

```
[root@VM_4_12_centos prometheus-2.3.2]# cat rules/jvmPerformanceAlert.rules 
groups:
- name: jvmPerformanceAlert
  rules:
  - alert: jvmMemoryUsageAlert
    expr: increase(jvm_memory_pool_bytes_used{pool="PS Old Gen",}[60s])/jvm_memory_pool_bytes_max{pool="PS Old Gen",}*100 > 70
    for: 15s
    labels:
      severity: page
    annotations:
      summary: "Instance {{ $labels.instance }} JVM memory usgae high"
      description: "{{ $labels.instance }} JVM memory usage above 70% (current value: {{ $value }})"
```

reload服务

curl -X POST http://10.90.4.12:9090/-/reload