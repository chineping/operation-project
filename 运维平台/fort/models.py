...
class Host(models.Model):
    ...
    host_name = models.CharField(max_length=50, unique=True, verbose_name='远程主机名')
    ip = models.GenericIPAddressField(verbose_name='主机IP')  # ip可能重复
    port = models.IntegerField(default=22, verbose_name='端口')
    release = models.CharField(max_length=50, default='CentOS', verbose_name='发行版本')
    cpu = models.CharField(max_length=20,verbose_name="CPU核数",null=True, blank=True)
    memory = models.CharField(max_length=20,verbose_name="内存容量",null=True, blank=True)
...
class RemoteUser(models.Model):
    ...
    # 用户名可以重复的！
    remote_user_name = models.CharField(max_length=50, verbose_name='远程主机用户名')
    # 密码将被用于ssh登录
...
class Group(models.Model):
    group_name = models.CharField(max_length=50, unique=True, verbose_name='堡垒机用户组名')
...
class UserProfile(models.Model):
    ...
    user_type = models.CharField(max_length=50, choices=(('0','普通用户'),('1','超级用户')), default='0', verbose_name='用户类型')
...
class AccessLog(models.Model):
    ...
    log_type = models.CharField(max_length=50, choices=log_type_choices, default='1', verbose_name='日志类型')
...
class JenkinsList(models.Model):
    job_name = models.CharField(max_length=50, unique=True, verbose_name='Jenkins项目名')
    build_url = models.CharField(max_length=50, verbose_name='项目url')

    def __str__(self):
        return '<%s>  [%s] ' % (self.job_name, self.build_url)

    class Meta:
        verbose_name = 'Jenkins列表'
